...
const confirmDelete = async () => {
  try {
    for (const patientId of selectedPatients) {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to delete patient with ID ${patientId}`);
      }
    }
    ...
    // Show error in popup
    setError(error instanceof Error ? error.message : 'Failed to delete patient(s). Please try again.');
    ...
    try {
      const response = await fetch('/api/patients', {
        ...
        body: JSON.stringify({
          ...
          facilityId: 2, // Explicitly provide a value for facilityId
        }),
      });
  ...
  return (
    <div className="p-10 max-w-[60rem] mx-auto">
      ...
      {/* Success Message */}
      {success && <p className="text-green-500 text-center mb-4">Patient added successfully!</p>}
            ...
            <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  { label: 'Cannabis Certificate Issued On', name: 'KetissuedOn', type: 'date' },
                  { label: 'Cannabis Certificate Expires At', name: 'KetexpiresOn', type: 'date' },
                  {
                    label: 'Race',
                    name: 'race',
                    type: 'select',
                  ...
                  <div key={name}>
                    ...
                    {type === 'select' ? (
                      <select
                        ...
                        onChange={(e) =>
                          setNewPatient({ ...newPatient, [name as keyof NewPatient]: e.target.value })
                        }
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="text-gray-700">
            <tr>
              ...
              <th className="border-b border-gray-300 px-4 py-2 text-left">First Name</th>
              <th className="border-b border-gray-300 px-4 py-2 text-left">Last Name</th>
              <th className="border-b border-gray-300 px-4 py-2 text-left">Date of Birth</th>
          ...
          <tbody className="text-gray-600">
            {filteredPatients.map((patient) => (
              <tr key={patient.id} className="hover:bg-gray-50 transition">
                ...
                <td className="border-b border-gray-300 px-4 py-2">{patient.firstName}</td>
                <td className="border-b border-gray-300 px-4 py-2">{patient.lastName}</td>
                <td className="border-b border-gray-300 px-4 py-2">
                  {new Date(patient.dob).toLocaleDateString()}
                </td>
                <td className="border-b border-gray-300 px-4 py-2">{patient.email}</td>
